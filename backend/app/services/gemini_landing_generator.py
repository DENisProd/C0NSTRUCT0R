...
class GeminiLLMGenerator:
    ...
    def generate_landing(
        ...
        logger.info("Gemini request: prompt_len=%d categories=%s", len(prompt or ""), categories)
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.warning("Gemini 503/UNAVAILABLE on attempt %d: %s", attempt + 1, se)
                last_error = se
        ...
        logger.info("Gemini response text length: %d", len(text or ""))
        ...
        if last_error is not None:
            meta["gemini_last_error"] = str(last_error)

        result = GenerateLandingResponse(
            blocks=blocks,
            palette=palette,
            meta=meta,
        )
        logger.info("Gemini parsed blocks=%d palette_keys=%d", len(blocks), len(palette_data or {}))
        return result
